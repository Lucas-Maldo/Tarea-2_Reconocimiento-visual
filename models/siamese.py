...
#         )(x)
    ...
    def get_test_encoder(self):       
        inputs = tf.keras.layers.Input((256, 256, self.CHANNELS))                
        x = inputs # / 127.5 - 1
        #the backbone can be an input to the clas SimSiam
        bkbone = resnet.ResNetBackbone([3,4,6,3], [64,128, 256, 512], kernel_regularizer = tf.keras.regularizers.l2(self.WEIGHT_DECAY))        
        x = bkbone(x)   
...
#         )(x)
    ...
    def build(self):
        photo = tf.keras.layers.Input(shape=(self.CROP_SIZE, self.CROP_SIZE, self.CHANNELS), name="photo")
        sketch = tf.keras.layers.Input(shape=(self.CROP_SIZE, self.CROP_SIZE, self.CHANNELS), name="sketch")
        negative = tf.keras.layers.Input(shape=(self.CROP_SIZE, self.CROP_SIZE, self.CHANNELS), name="negative")

        photo_embedding = self.encoder(photo)
        sketch_embedding = self.encoder(sketch)
        negative_embedding = self.encoder(negative)

        loss, _, _ = self.compute_loss(photo_embedding, sketch_embedding, negative_embedding)

        super(Siamese, self).__init__(inputs=[photo, sketch, negative], outputs=loss)